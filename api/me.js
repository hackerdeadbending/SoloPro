...
export default async function handler(req,res){
 ...
 try{
  const user=await authenticate(req);if(!user)return res.status(401).json({error:'Authentication required.'});
  ...
  const until=profile?.premium_until?Date.parse(profile.premium_until):NaN;const timeLimitedActive=Number.isFinite(until)&&until>Date.now();const premiumActive=Boolean(user.isAdmin||profile?.premium_active||timeLimitedActive);
