...
export default async function handler(req,res){
  ...
  try{
    ...
    const rows=await supabase(`/rest/v1/profiles?id=eq.${encodeURIComponent(user.id)}&smart_messages_trial_started_at=is.null&select=id`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({smart_messages_trial_started_at:now})});
    if(!rows?.[0]){
