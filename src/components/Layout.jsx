...
export default function Layout({children}){
 ...
 const ownerSignedIn=String(app.authSession?.user?.email||'').trim().toLowerCase()===ownerEmail;
 const nav=[['/','dashboard','grid'],['/earnings','earnings','dollar'],['/clients','clients','users'],['/tax','tax','file'],['/referral','referral','gift'],['/premium','premium','crown'],['/settings','settings','settings']];if(app.isAdmin||ownerSignedIn)nav.push(['/admin','Admin','settings']);
 const renderNav=()=>nav.map(([to,key,icon])=><NavLink key={to} to={to} end={to==='/' } onClick={()=>setMobileMenu(false)} className={({isActive})=>`nav-link ${isActive?'active ':''}${key==='settings'||key==='Admin'?'nav-link-settings':''}`}><span className="nav-icon"><Icon name={icon} size={19} strokeWidth={1.8}/></span><span>{key==='Admin'?key:t(key)}</span></NavLink>);
 const accountAuthenticated=Boolean(app.account?.authenticated||app.authSession?.user?.email);
