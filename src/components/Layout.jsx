...
export default function Layout({children}){
 ...
 return <div className="shell"><aside className="sidebar"><div className="brand"><span className="brand-mark"><Icon name="spark" size={16}/></span><span>SoloPro</span></div><div className="sidebar-label">{t('workspace')}</div><nav>{renderNav()}</nav><div className="sidebar-bottom">{accountCard}<div className="sidebar-note">{t('yourBusinessSimplified')}</div></div></aside><div className="main-wrap"><header className="topbar"><button className="mobile-menu-btn" aria-label="{t('openNavigation')}" aria-expanded={mobileMenu} onClick={()=>setMobileMenu(v=>!v)}><Icon name="menu" size={20}/></button><div className="mobile-brand">SoloPro</div><div className="topbar-spacer"/><span className="topbar-status">{t('privateWorkspace')}</span></header>{mobileMenu&&<div className="mobile-menu-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setMobileMenu(false)}><aside className="mobile-drawer"><div className="mobile-drawer-head"><strong>SoloPro</strong><button className="icon-btn" onClick={()=>setMobileMenu(false)} aria-label="{t('closeNavigation')}">×</button></div><nav>{renderNav()}</nav><div className="mobile-drawer-account">{accountCard}</div><div className="sidebar-note">{t('yourBusinessSimplified')}</div></aside></div>}{expiryWarning&&<div className="premium-expiry-banner"><Icon name="crown" size={16}/><span><strong>{t('premiumEndsTomorrow')}</strong> {t('renewKeepPremium')}</span><NavLink to="/premium">{t('renewPremium')}</NavLink></div>}<main className="content">{children}</main></div><AccountModal open={accountOpen} onClose={()=>setAccountOpen(false)}/><style>{`.nav-link .nav-icon{width:19px;height:19px;display:grid;place-items:center;flex:0 0 19px}.nav-link-settings .nav-icon{transform:scale(1.12);transform-origin:center}.nav-link-settings,.nav-link-settings::before,.nav-link-settings::after{border-top:0!important;border-bottom:0!important;box-shadow:none!important}.nav-link-settings::after{content:none!important}.nav-link-smart-messenger{display:flex!important;visibility:visible!important;opacity:1!important}.premium-expiry-banner{margin:12px 18px 0;display:flex;align-items:center;gap:10px;padding:10px 13px;border:1px solid rgba(236,61,152,.25);border-radius:12px;background:rgba(236,61,152,.07);color:#d9d9df;font-size:11px;line-height:1.45}.premium-expiry-banner svg{color:var(--pink);flex:0 0 auto}.premium-expiry-banner span{flex:1;min-width:0}.premium-expiry-banner strong{color:#fff}.premium-expiry-banner a{color:#fff;text-decoration:none;font-weight:700;white-space:nowrap}@media(max-width:760px){.premium-expiry-banner{margin:10px 12px 0;align-items:flex-start;flex-wrap:wrap}.premium-expiry-banner a{margin-left:26px}}`}</style></div>;
}