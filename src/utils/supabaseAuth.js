...
export async function refreshSession(session){if(!session?.refresh_token)return null;try{const data=await request('/auth/v1/token?grant_type=refresh_token',{method:'POST',headers:headers(session.access_token)});const next={access_token:data.access_token,refresh_token:data.refresh_token,user:data.user};saveSession(next);return next;}catch{saveSession(null);return null;}}
export function getUser(accessToken){return request('/auth/v1/user',{headers:headers(accessToken)});}
function emitPremiumSync(data){try{window.dispatchEvent(new CustomEvent('solopro:premium-sync',{detail:data}));}catch{}}
export async function getServerProfile(accessToken){const fetchProfile=async token=>{const res=await fetch(`/api/me?sync=${Date.now()}`,{cache:'no-store',headers:{Authorization:`Bearer ${token}`,'Cache-Control':'no-cache'}});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data?.error||'Unable to load account security state.');return data;};const data=await fetchProfile(accessToken);lastPremiumState=Boolean(data?.premiumActive);emitPremiumSync(data);if(!presenceTimer){const ping=()=>{const session=getStoredSession();const token=session?.access_token;if(token)fetch('/api/presence',{method:'POST',headers:{Authorization:`Bearer ${token}`}}).catch(()=>{});};ping();presenceTimer=setInterval(ping,60000);}if(!premiumSyncTimer){const syncPremium=async()=>{const session=getStoredSession();const token=session?.access_token;if(!token)return;try{const latestData=await fetchProfile(token);if(!latestData||typeof latestData.premiumActive!=='boolean')return;const nextPremium=Boolean(latestData.premiumActive);lastPremiumState=nextPremium;emitPremiumSync(latestData);}catch{}};premiumSyncTimer=setInterval(syncPremium,3000);}return data;}
export async function signOut(accessToken){if(authConfigured&&accessToken){try{await fetch(`${SUPABASE_URL}/auth/v1/logout`,{method:'POST',headers:headers(accessToken)});}catch{}}if(presenceTimer){clearInterval(presenceTimer);presenceTimer=null;}if(premiumSyncTimer){clearInterval(premiumSyncTimer);premiumSyncTimer=null;}lastPremiumState=null;saveSession(null);}
