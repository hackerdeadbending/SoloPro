...
function request(path,options={}){
  if(!authConfigured) throw new Error('Account services are temporarily unavailable. Please try again later.');
  return fetch(`${SUPABASE_URL}${path}`,options).then(async res=>{const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data?.msg||data?.error_description||data?.message||data?.error||'Authentication request failed.');return data;});
}

function getReferralAttribution(){
  const fromUrl=String(new URLSearchParams(window.location.search).get('ref')||'').trim();
  if(fromUrl){try{localStorage.setItem('solopro-referral-attribution',fromUrl);}catch{} return fromUrl;}
  try{return String(localStorage.getItem('solopro-referral-attribution')||'').trim();}catch{return '';}
}

export async function signUp({name,email,password}){
  const referralCode=getReferralAttribution();
  const data=await request('/auth/v1/signup',{method:'POST',headers:headers(),body:JSON.stringify({email,password,data:{full_name:name,referral_code:referralCode}})});
  if(!data.access_token)return {needsConfirmation:true,user:data.user};
  const session={access_token:data.access_token,refresh_token:data.refresh_token,user:data.user};saveSession(session);return session;
}
...
export async function getServerProfile(accessToken){const res=await fetch('/api/me',{headers:{Authorization:`Bearer ${accessToken}`}});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data?.error||'Unable to load account security state.');return data;}
export async function signOut(accessToken){if(authConfigured&&accessToken){try{await fetch(`${SUPABASE_URL}/auth/v1/logout`,{method:'POST',headers:headers(accessToken)});}catch{}}saveSession(null);}
