...
export default function Referral(){
  ...
  const remainingText=remaining===1?t('oneReferralRemaining'):t('referralsRemaining').replace('{count}',String(remaining));
  ...
  return <div className="page referral-page">
    <style>{`
      ...
      .referral-page button{transition:transform .16s ease,opacity .16s ease,box-shadow .2s ease}.referral-page button:not(:disabled):hover{transform:translateY(-1px)}
      ...
      .stats{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:12px}.stat{padding:13px 14px;border-radius:15px;background:rgba(255,255,255,.022);border:1px solid rgba(255,255,255,.055);display:grid;grid-template-columns:29px 1fr;grid-template-rows:auto auto;column-gap:9px;row-gap:3px;align-items:start;min-height:72px}.mini-icon{width:29px;height:29px;border-radius:9px;grid-row:1/3}.stat-label{font-size:10px;line-height:1.25;opacity:.45;min-height:25px;display:flex;align-items:flex-start}.stat strong{font-size:19px;line-height:1.1;letter-spacing:-.03em;transform:translateY(3px)}
      .link-panel{padding:18px 20px!important}.link-head{display:flex;justify-content:space-between;align-items:flex-start;gap:14px}.link-copy{display:flex;gap:12px;align-items:flex-start}.link-icon{width:39px;height:39px;border-radius:12px}.link-actions{display:flex;gap:7px;flex-shrink:0}.share-button{display:inline-flex!important;align-items:center;gap:9px!important;min-height:42px;padding:0 17px!important;border-radius:13px!important;font-weight:700!important;letter-spacing:-.01em;box-shadow:0 8px 24px rgba(0,0,0,.16),inset 0 1px 0 rgba(255,255,255,.12);position:relative;overflow:hidden}.share-button:before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,.12),transparent 55%);pointer-events:none}.share-button svg{position:relative;z-index:1}.share-button span{position:relative;z-index:1}.share-button:not(:disabled):hover{transform:translateY(-2px)!important;box-shadow:0 12px 28px rgba(0,0,0,.22),inset 0 1px 0 rgba(255,255,255,.15)}.link-actions button{display:inline-flex;align-items:center;gap:6px;white-space:nowrap}.referral-link-box{display:flex;align-items:center;gap:10px;margin-top:14px;padding:11px 11px 11px 14px;border-radius:12px;background:rgba(0,0,0,.13);border:1px solid rgba(255,255,255,.075);min-width:0}.referral-link-box code{font:500 11px/1.3 ui-monospace,SFMono-Regular,Menlo,monospace;opacity:.72;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1;min-width:0}.copy-icon{width:32px;height:32px!important;padding:0!important;border-radius:9px!important;display:grid!important;place-items:center!important;flex:0 0 auto}.link-note{font-size:10px;opacity:.4;margin-top:9px;display:flex;gap:6px;align-items:center}
    ...
    <div className="referral-main">
      <section className="panel reward-card">
        <div className="reward-top">
          <div className="reward-copy"><div className="reward-icon"><Icon name={milestones>0?'check':'gift'} size={19}/></div><div className="reward-title"><div className="eyebrow">{t('nextReward')}</div><h2>{verified===0?t('startFirstReward'):progress===0?t('milestoneComplete').replace('{count}',String(milestones)):remainingText}</h2><p>{verified===0?t('reachSevenVerified'):progress===0?t('milestoneCompletedKeepSharing'):t('everyVerifiedCloser')}</p></div></div>
          <div className="reward-number">{progress===0&&verified>0?7:progress}<span> / 7</span></div>
        </div>
        <div className="meter"><div className="meter-head"><span>{t('progress')}</span><span>{verified} {t('verifiedTotal')}</span></div><div className="meter-bar"><i style={{width:`${progressPercent}%`}}/></div><div className="meter-foot"><span>{progress===0&&verified>0?t('milestoneCompleteShort'):t('remainingCount').replace('{count}',String(remaining))}</span><strong>{milestones} {t('rewardsEarnedLabel')}</strong></div></div>
      </section>
    ...
    <div className="stats">
      <Stat icon="users" label={t("verifiedNewUsers")} value={verified}/><Stat icon="send" label={t("invitesSent")} value={invited}/><Stat icon="gift" label={t("rewardsEarned")} value={milestones}/><Stat icon="crown" label={t("availableNow")} value={available}/>
    </div>
