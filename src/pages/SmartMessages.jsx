...
function load(email){try{return JSON.parse(localStorage.getItem(storageKey(email)))||{items:[],channels:{}}}catch{return {items:[],channels:{}}}}
...
function money(app,value){if(value===''||value==null)return '';return new Intl.NumberFormat(app.country.locale,{style:'currency',currency:app.currencyCode||'EUR',maximumFractionDigits:2}).format(Number(value)||0)}
...
export default function SmartMessages(){
 const app=useApp();
 const email=app.account?.email||app.user?.email||'';
 const authenticated=Boolean(app.account?.authenticated);
 const [trial,setTrial]=useState(()=>getSmartTrialStatus(email));
 const [data,setData]=useState(()=>load(email));
 const [form,setForm]=useState(blank);
 const [selected,setSelected]=useState(null);
 const [status,setStatus]=useState('');
 const clients=app.clients||[];
 const premium=app.isAdmin||app.premiumActive||trial.active;
 useEffect(()=>{setTrial(getSmartTrialStatus(email));setData(load(email))},[email]);
 useEffect(()=>{if(authenticated)save(email,data)},[email,data,authenticated]);
 const upcoming=useMemo(()=>[...data.items].sort((a,b)=>`${a.date} ${a.time}`.localeCompare(`${b.date} ${b.time}`)),[data.items]);
 const selectedItem=selected?data.items.find(x=>x.id===selected):null;
 const selectedClient=clients.find(x=>x.id===selectedItem?.clientId);
 const updateForm=(key,value)=>setForm(v=>({...v,[key]:value}));
 const addItem=e=>{e.preventDefault();if(!form.clientId)return;const item={...form,id:`sm-${Date.now()}`};setData(d=>({...d,items:[...d.items,item]}));setForm(blank);setSelected(item.id);setStatus('Added to your week. SoloPro will build the message from the details you gave it.')};
 const remove=id=>{setData(d=>({...d,items:d.items.filter(x=>x.id!==id)}));if(selected===id)setSelected(null)};
 const start=()=>{const next=startSmartTrial(email);setTrial(next)};
 if(!premium){return <div className="page"><div className="page-top"><div><div className="eyebrow">SOLOPRO PREMIUM</div><h1>Smart client messages.</h1><p className="sub">Plan your week, add only the details you know, and SoloPro turns each note into a polished client message.</p></div></div><section className="panel" style={{maxWidth:760}}><div className="smart-lock"><div className="smart-lock-icon"><Icon name="spark" size={24}/></div><h2>Try it free for 7 days</h2><p>Smart Messages is a Premium workspace for client follow-ups, appointments, prices and practical updates. Your one-time trial lasts 7 days and is available once per account.</p>{!authenticated?<><strong>Create a free account to start your one-time trial.</strong><p className="modal-sub">You can keep using the rest of SoloPro as a guest.</p></>:trial.started?<><strong>Your 7-day trial has ended.</strong><p className="modal-sub">Upgrade to Premium to keep Smart Messages active.</p></>:<button className="primary" onClick={start}>Start my 7-day trial</button>}</div></section></div>}
 return <div className="page"><div className="page-top"><div><div className="eyebrow">SMART MESSAGES</div><h1>Your week, ready to send.</h1><p className="sub">Add a client and the details you know. SoloPro shapes a natural message automatically — appointment, task, price and note included only when relevant.</p></div><div className="smart-trial-pill">{app.premiumActive||app.isAdmin?'Premium active':`${trial.daysLeft} days left in trial`}</div></div>
  <div className="smart-layout">
   <section className="panel"><div className="panel-head"><div><h2>Add to the week</h2><p>Nothing is mandatory except the client.</p></div></div><form className="form-stack" onSubmit={addItem}>
