...
export default function SmartMessages(){
 ...
 const channelLabel={whatsapp:'WhatsApp',sms:'SMS',telegram:'Telegram',instagram:'Instagram',email:'Email',copy:t('copyMessage')}[selectedChannel]||t('sendMessage');
 ...
 const add=e=>{e.preventDefault();if(!form.clientId)return;const item={...form,id:`sm-${Date.now()}`};setItems(x=>[...x,item]);openDetail(item.id);setForm({...empty,date:new Date().toISOString().slice(0,10)});setNotice(t('smartPreparedNotice'))};
 const start=async()=>{if(!app.authSession?.access_token){setNotice(t('smartRefreshSession'));return}setTrialBusy(true);try{const r=await fetch('/api/smart-trial',{method:'POST',headers:{Authorization:`Bearer ${app.authSession.access_token}`}});const p=await r.json().catch(()=>({}));if(!r.ok&&r.status!==409)throw new Error(p.error||t('smartTrialStartError'));const at=p.startedAt||Date.now();setTrial(startSmartTrial(email,at));setNotice(r.status===409?t('smartTrialAlreadyUsed'):t('smartTrialActive'))}catch(e){setNotice(e.message||t('smartTrialStartError'))}finally{setTrialBusy(false)}};
 const openChannel=channel=>{if(!message)return;if(channel==='copy'){navigator.clipboard?.writeText(message);setNotice(t('messageCopied'));return}if(channel==='sms'){const phone=client?.socialChannel==='sms'&&client?.socialHandle?client.socialHandle:client?.phone;if(!phone){setNotice(t('addPhoneFirst'));return}window.location.href=`sms:${String(phone).replace(/[^0-9+]/g,'')}?body=${encodeURIComponent(message)}`;return}if(channel==='whatsapp'){const phone=client?.socialChannel==='whatsapp'&&client?.socialHandle?client.socialHandle:client?.phone;if(!phone){setNotice(t('addWhatsappFirst'));return}window.open(`https://wa.me/${String(phone).replace(/[^0-9]/g,'')}?text=${encodeURIComponent(message)}`,'_blank','noopener,noreferrer');return}if(channel==='email'){if(!client?.email){setNotice(t('addEmailFirst'));return}window.location.href=`mailto:${client.email}?subject=${encodeURIComponent('A quick note from SoloPro')}&body=${encodeURIComponent(message)}`;return}const handle=client?.socialChannel===channel?String(client?.socialHandle||'').trim().replace(/^@/,''):'';if(!handle){setNotice(`${t('addChannelFirst')} ${channel}`);return}navigator.clipboard?.writeText(message);window.open(`${channel==='telegram'?'https://t.me/':'https://instagram.com/'}${encodeURIComponent(handle)}`,'_blank','noopener,noreferrer');setNotice(`${t('messageCopiedOpen')} ${channel}`)};
