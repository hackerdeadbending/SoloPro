...
export default function Earnings(){
 ...
 return <div className="page">
  ...
  <div className="panel">
   ...
   {services.length===0?<div className="empty"><Icon name="dollar" size={25}/><strong>No services yet</strong><span>Add a service, or use Additional income for money that does not come from client work.</span></div>:<div className="table-list">{services.map(x=><div className="table-row detailed" key={x.id}><div><strong>{x.service}</strong><small>{x.clientName||'Unassigned'} — {new Date(x.date).toLocaleString()}</small></div><span>{money(x.amount)}</span><small>Costs {money((x.materialCost||0)+(x.extraExpense||0))}</small><b>{money((x.amount||0)-(x.materialCost||0)-(x.extraExpense||0))}</b><button className="icon-btn danger" onClick={()=>app.deleteService(x.id)}><Icon name="trash" size={16}/></button></div>)}</div>}
  </div>
